  ...
  class ApexAutomation {
    ...
    async startAutomation(data, sendResponse) {
      ...
      try {
        ...
        this.processAccounts().catch(error => {
    ...
    async processAccounts() {
      for (let i = 1; i <= this.settings.numberOfAccounts; i++) {
        ...
        this.sendProgressUpdate(i, this.settings.numberOfAccounts);

        try {
          await this.processSingleAccount(i);
    ...
    async processSingleAccount(accountNumber) {
      ...
      if (currentUrl.includes('/payment/') || currentUrl.includes('/authorize-cim')) {
        ...
        await this.waitForConfirmation(accountNumber);
    ...
    humanDelay(minMs = 1000, maxMs = 3000) {
    ...
    async typeWithDelay(element, text, delayMs = 100) {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        await this.delay(delayMs + Math.random() * 50); // 100-150ms per character
      }
    }
    ...
    async checkForContinuation() {
      ...
      try {
        const result = await chrome.storage.local.get(['apexAutomationState', 'apexAutomationLock']);
        if (result.apexAutomationState && result.apexAutomationState.isRunning) {
          ...
          const timeSinceLastUpdate = Date.now() - result.apexAutomationState.timestamp;
          ...
          if (!window.apexAutomationRunning && !this.isRunning && timeSinceLastUpdate < 30000) {
            ...
            if (!result.apexAutomationLock) {
              ...
              this.settings = result.apexAutomationState.settings;
              this.currentAccount = result.apexAutomationState.currentAccount;

              // Continue with the current account
              await this.processSingleAccount(this.currentAccount);
            } else {
              this.addLog('Another instance already has the automation lock, skipping continuation');
            }
          } else {
